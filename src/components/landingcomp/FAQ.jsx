...
export default function FAQ() {
  ...
  return (
    ...
    >
      {/* ── consistent 48px padding on all sides, safe on every screen ── */}
      <div style={{ maxWidth:'1380px', margin:'0 auto', padding:'0 48px' }}>

        {/*
          Responsive grid via inline style + a <style> block:
          • Mobile/Tablet (<1024px): 1 column, header above accordion
          • Desktop       (≥1024px): 2 columns, left header sticky
        */}
        <style>{`
          #faq-grid {
            display: grid;
          ...
          /* Tablet: single column, tighter padding */
          @media (max-width: 1024px) {
            #faq-grid  { grid-template-columns: 1fr; gap: 48px; }
            #faq-left  { position: static; }
          }

          /* Mobile: tighten outer padding so heading never clips */
        ...
        <div id="faq-grid">
          ...
          >
            ...
            <h2
              id="faq-title"
              style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'clamp(2.8rem,5.5vw,5rem)', letterSpacing:'0.04em', color:'#0A0A0A', lineHeight:0.95, marginBottom:'24px' }}
            >
...
function FAQItem({ faq, index, isOpen, onToggle }) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontFamily:"'Manrope',sans-serif", fontWeight:600, fontSize:'0.97rem', color:'#0A0A0A', lineHeight:1.5 }}>
          {faq.q}
        </span>
