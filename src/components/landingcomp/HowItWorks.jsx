    ...
    <section id="how-it-works" style={{ padding:'120px 48px', background:'#FAFAFA', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))', gap:32, position:'relative' }}>
          <div style={{ position:'absolute', top:60, left:'12.5%', right:'12.5%', height:1,
          ...
          {steps.map((s, i) => (
            <div key={i} className="reveal" style={{
              ...
              onMouseEnter={e => { e.currentTarget.style.transform='translateY(-8px)'; e.currentTarget.style.boxShadow='0 16px 48px rgba(255,107,0,0.12)' }}
              onMouseLeave={e => { e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.boxShadow='0 4px 24px rgba(0,0,0,0.04)' }}
            >
              <div style={{ position:'absolute', top:20, right:24, fontFamily:"'Bebas Neue',sans-serif",
                fontSize:'3.5rem', color:'rgba(255,107,0,0.06)', letterSpacing:'0.05em', lineHeight:1 }}>{s.num}</div>
              <div style={{ width:52, height:52, borderRadius:16, background:'#FFF5EE',
                display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, marginBottom:20 }}>{s.icon}</div>
              <h3 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'1.6rem', letterSpacing:'0.04em',
                color:'#0A0A0A', marginBottom:12 }}>{s.title}</h3>
